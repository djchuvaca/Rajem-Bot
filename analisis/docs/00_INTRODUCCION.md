...
- **Qué es:** SQLite compilado a WebAssembly — corre completamente en el proceso de Node.js
- **Cómo persiste:** La BD vive en RAM. Cada escritura dispara un debounce de 500ms que exporta el archivo binario a disco (`data/tacos_javier.db`)
- **Ventajas:** Cero configuración, portable, sin servidor de BD separado
- **Limitaciones:** Si el proceso muere sin flushar, puede perder las últimas escrituras del buffer de 500ms. Para una taquería el riesgo es mínimo.
...
carnitas-bot 1.4/
│
├── index.js                    ← Punto de entrada: inicializa BD, panel, bot WA
├── package.json
...
│   │
│   ├── db/
│   │   ├── core.js             ← Motor BD: initDB, run, queryOne, queryAll, guardarDB
...
│   │
│   ├── panel/
│   │   ├── server.js           ← Express app: rutas API, autenticación, panel
│   │   ├── whatsapp-bridge.js  ← Singleton del cliente WA (evita deps circulares)
│   │   └── public/
│   │       └── index.html      ← SPA del panel (auto-refresh cada 20s)
│   │
...
```env
GROQ_API_KEY=gsk_...         # Clave de API de Groq (obligatoria para fallback IA)
GRUPO_ID=521XXXXXXXXXX@g.us  # JID del grupo de WhatsApp de administración
PANEL_PORT=3000               # Puerto del panel web (opcional, default 3000)
PANEL_SECRET=...              # Secreto para firmar cookies de sesión del panel
```
...
**Nota:** Después de un reinicio del bot, WhatsApp Web tarda 15–30 segundos en reconectarse. Durante ese tiempo no se procesan mensajes.
...
Cliente WhatsApp
 ...
 Estado en memoria (Maps)
      │  al cambiar estado
      ▼
 sql.js → guardarDB() debounced 500ms → tacos_javier.db
      │
      ▼
